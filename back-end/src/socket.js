...
const SingleMintOnSale_monitor = async (blockNumber, toBlockNumber) => {
  try {
      var event = await myContract.getPastEvents("SingleMintOnSale", { fromBlock: blockNumber, toBlock: toBlockNumber });
      if (event.length > 0) {
          let i;
...
const DestroySale_monitor = async (blockNumber, toBlockNumber) => {
  try {
      var event = await myContract.getPastEvents("DestroySale", { fromBlock: blockNumber, toBlock: toBlockNumber });
      if (event.length > 0) {
          let i;
          for (i = 0; i < event.length; i++) {
              let data = event[i];
...
const PlaceBid_monitor = async (blockNumber, toBlockNumber) => {
  try {
      var event = await myContract.getPastEvents("PlaceBid", { fromBlock: blockNumber, toBlock: toBlockNumber});
      if (event.length > 0) {
          let i;
          for (i = 0; i < event.length; i++) {
              let data = event[i];
...
const AcceptBid_monitor = async (blockNumber, toBlockNumber) => {
  try {
      var event = await myContract.getPastEvents("AcceptBid", { fromBlock: blockNumber, toBlock: toBlockNumber});
      if (event.length > 0) {
          let i;
          for (i = 0; i < event.length; i++) {
              let data = event[i];
...
const BuyNow_monitor = async (blockNumber, toBlockNumber) => {
  try {
      var event = await myContract.getPastEvents("BuyNow", { fromBlock: blockNumber, toBlock: toBlockNumber });
      if (event.length > 0) {
          let i;
          for (i = 0; i < event.length; i++) {
              let data = event[i];
...
const EndBid_monitor = async (blockNumber, toBlockNumber) => {
  try {
      var event = await myContract.getPastEvents("EndBid", { fromBlock: blockNumber, toBlock: toBlockNumber });
      if (event.length > 0) {
          let i;
          for (i = 0; i < event.length; i++) {
              let data = event[i];
            ...
            for (i = 0; i < event.length; i++) {
                ...
                if (compareObjects(BurnTemp, objTemp) === false) {
                  ...
                  Item.findOneAndDelete({ _id: new ObjectId(tokenHash) }).then((data) => {
                      ...
                      io.sockets.emit("UpdateStatus", { type: "BURN_NFT", data : {itemId: data._id, colId : data.collection_id}});
...
const BatchMintOnSale_monitor = async (blockNumber, toBlockNumber) => {
  try {
      var event = await myContract.getPastEvents("BatchMintOnSale", { fromBlock: blockNumber, toBlock: toBlockNumber });
      if (event.length > 0) {
          let i;
          for (i = 0; i < event.length; i++) {
              let data = event[i];
...
const BatchEndAuction_monitor = async (blockNumber, toBlockNumber) => {
  try {
      var event = await myContract.getPastEvents("BatchEndAuction", { fromBlock: blockNumber, toBlock: toBlockNumber });
      if (event.length > 0) {
          let i;
          for (i = 0; i < event.length; i++) {
              let data = event[i];
